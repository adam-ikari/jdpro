...
!(async () => {
    if (!cookiesArr[0]) {
        $.msg($.name, '【提示】请先获取京东账号一cookien直接使用NobyDa的京东签到获取', 'https://bean.m.jd.com/bean/signIndex.action', {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});
        return;
    }
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.tryCount = 0;
            //await TotalBean();
            console.log(`n******开始【京东账号${$.index}】${$.nickName || $.UserName}*********n`);
            if (!$.isLogin) {
                $.msg($.name, `【提示】cookie已失效`, `京东账号${$.index} ${$.nickName || $.UserName}n请重新登录获取nhttps://bean.m.jd.com/bean/signIndex.action`, {"open-url": "https://bean.m.jd.com/bean/signIndex.action"});

                if ($.isNode()) {
                    await notify.sendNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}n请重新登录获取cookie`);
                }
                continue
            }
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.flag) {
                            ...
                            //     $.tryCount++;
                            // } else {
                                //message += `保价失败：${data.responseMessage}n`;
                            // }
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.flag) {
                            ...
                            message += `保价成功：返还${data.succAmount}元n`
                        } else {
...
function showMsg() {
    return new Promise(resolve => {
        if (message) {
            allMessage += `【京东账号${$.index}】${$.nickName || $.UserName}n${message}${$.index !== cookiesArr.length ? 'nn' : 'nn'}`;
        }
        $.msg($.name, '', `【京东账号${$.index}】${$.nickName || $.UserName}n${message}`);
        resolve()
    })
}
